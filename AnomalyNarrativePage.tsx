...
const AnomalyNarrativePage: React.FC = () => {
  ...
  // Data fetching
  const { anomaly, transaction, loading, error, transactionId } =
    useAnomalyData();

  // UI state
  const [selectedPersona, setSelectedPersona] =
    useState<string>("fraud-analyst");
  ...
  // Narrative translations (English only) - fallback when SHAP narrative hasn't loaded
  const aiNarratives = NARRATIVE_TRANSLATIONS.en;

  // Live narratives from API — overrides static translations when available
  const liveNarratives: Record<string, { title: string; content: string }> =
    shapNarrative
      ? [
          "fraud-analyst",
          "compliance-officer",
  ...
  const generateRiskContribution = () => {
    ...
    return reasons.map((reason, index) => ({
      factor:
        reason.description || reason.reasonCode || `Risk Factor ${index + 1}`,
  ...
  const handleDownloadPDF = () => {
    ...
    setTimeout(() => {
      try {
        ...
        doc.text("BLOCKCHAIN ANOMALY INTELLIGENCE REPORT", pageWidth / 2, 20, {
          align: "center",
        });
        doc.setFontSize(10);
        doc.text("Confidential - For Internal Use Only", pageWidth / 2, 30, {
          align: "center",
        });
        ...
        addText(
          `Risk Score: ${transactionData.riskScore}/100 - Very High Risk`,
          11,
          true,
          [220, 38, 38],
        );
        ...
        addText(liveNarratives["fraud-analyst"].content, 10);
        ...
        autoTable(doc, {
          ...
          body: riskContribution.map((item) => [
            item.factor,
            `${item.percentage}%`,
          ]),
          theme: "striped",
        ...
        // Download PDF
        doc.save(
          `Anomaly_Report_${transactionData.id}_${new Date().toISOString().split("T")[0]}.pdf`,
        );
  ...
  return (
    ...
    >
      ...
      <TransactionHeaderCard anomaly={anomaly} transaction={transaction} />
            ...
            <Alert
              ...
              description={
                <div className="flex items-center gap-2">
                  <Spin size="small" />
                  <span>
                    Using SHAP explainability and Cortex AI to generate
                    insights...
                  </span>
                </div>
              }
