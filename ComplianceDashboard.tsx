...
const ComplianceDashboard: React.FC = () => {
  ...
  const [analysisResult, setAnalysisResult] =
    useState<TransactionAnalysis | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-600">
              Screen transactions and detect anomalies
            </p>
          </div>

          {/* Search Section */}
          <div className="mb-6">
            <h2 className="text-xl hidden font-semibold text-gray-900 mb-4">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <InfoCard title="Transaction Analysis Results">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <DataCard
                        ...
                        value={
                          analysisResult.transactionHash.substring(0, 20) +
                          "..."
                        }
                      />
                      <DataCard
                        label="Amount"
                        value={`${analysisResult.currency} ${(analysisResult.amount || 0).toLocaleString()}`}
                      />
                      <DataCard
                        label="From"
                        value={analysisResult.from.substring(0, 15) + "..."}
                      />
                      <DataCard
                        label="To"
                        value={analysisResult.to.substring(0, 15) + "..."}
                      />
                      <DataCard
                        label="Timestamp"
                        value={new Date(
                  ...
                  <div
                    className={`rounded-lg p-4 ${analysisResult.riskScore > 70 ? "bg-red-50 border border-red-200" : "bg-yellow-50 border border-yellow-200"}`}
                  >
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    ...
                    <div className="space-y-2">
                      <button
                        onClick={() =>
                          analysisResult?.transactionHash &&
                          navigate(
                            `/narrative/${analysisResult.transactionHash}`,
                          )
                        }
        ...
        {/* Scanning animation — shown while API call is in progress */}
        {isAnalyzing && (
          <div className="mt-8 border border-blue-200 rounded-xl bg-white py-14 flex flex-col items-center justify-center text-center px-6">
            <div className="relative w-20 h-20 mb-6">
              {/* Outer pulse ring */}
              <span className="absolute inset-0 rounded-full bg-blue-100 animate-ping opacity-60" />
              {/* Middle ring */}
              <span
                className="absolute inset-2 rounded-full bg-blue-200 animate-ping opacity-40"
                style={{ animationDelay: "0.3s" }}
              />
              {/* Icon center */}
              <span className="absolute inset-0 flex items-center justify-center">
                <svg
                  className="w-9 h-9 text-blue-500 animate-pulse"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <p className="text-gray-400 text-sm max-w-xs">
              Running all 6 anomaly detectors. This will only take a moment.
            </p>
            {/* Animated dots */}
        ...
        {/* Placeholder — shown when no analysis has been run yet */}
        {!analysisResult && !isAnalyzing && (
          <div className="mt-8 border border-dashed border-gray-300 rounded-xl bg-white py-14 flex flex-col items-center justify-center text-center px-6">
            ...
            <p className="text-gray-400 text-sm max-w-sm">
              Enter a transaction hash above and click{" "}
              <span className="font-medium text-gray-600">Analyze</span> to run
              a scan — or use{" "}
              <span className="font-medium text-gray-600">Upload CSV</span> to
