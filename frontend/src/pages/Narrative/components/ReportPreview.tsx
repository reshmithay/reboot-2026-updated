...
interface ReportPreviewProps {
    ...
    riskContribution: Array<{ factor: string; percentage: number; color: string }>;
}
...
export const ReportPreview: React.FC<ReportPreviewProps> = ({
    ...
    return (
        <div style={{ maxHeight: "70vh", overflowY: "auto", padding: "20px" }}>
            {/* Report Header */}
            <div style={{ 
                background: "linear-gradient(135deg, rgb(24 125 57) 0%, rgb(30, 64, 175) 100%)", 
                color: "white", 
                padding: "30px", 
                borderRadius: "8px 8px 0 0",
                marginBottom: "24px"
            }}>
                <h1 style={{ color: "white", margin: 0, fontSize: "24px", fontWeight: "bold", textAlign: "center" }}>
                    BLOCKCHAIN ANOMALY INTELLIGENCE REPORT
                </h1>
                <p style={{ color: "rgba(255,255,255,0.9)", textAlign: "center", margin: "8px 0 0 0", fontSize: "12px" }}>
            ...
            <Card title="TRANSACTION DETAILS" size="small" style={{ marginBottom: 16 }}>
                <Descriptions column={2} size="small" bordered>
                    <Descriptions.Item label="Date & Time">{transactionData.date}</Descriptions.Item>
                    <Descriptions.Item label="Amount">{transactionData.amount}</Descriptions.Item>
                ...
                <Space direction="vertical" style={{ width: "100%" }} size="small">
                    {riskIndicators.map((indicator, idx) => (
                        <div 
                            ...
                            style={{ 
                                padding: "12px", 
                                background: indicator.severity === "high" ? "#fff1f0" : "#fffbe6",
                                border: `1px solid ${indicator.severity === "high" ? "#ffa39e" : "#ffe58f"}`,
                                borderRadius: "4px"
                            }}
                        >
                            <div style={{ fontWeight: 600, marginBottom: 4, color: indicator.severity === "high" ? "#cf1322" : "#d46b08" }}>
                                {idx + 1}. [{indicator.severity.toUpperCase()}] {indicator.label}
                            </div>
            ...
            <Card title="RECOMMENDED ACTIONS" size="small" style={{ marginBottom: 16 }}>
                {recommendedActions.map((action) => (
                    <div key={action.priority} style={{ marginBottom: 16 }}>
                        <h4 style={{ 
                            color: action.priority === "immediate" ? "#cf1322" : action.priority === "within-1-hour" ? "#d46b08" : "#389e0d",
                            marginBottom: 8
                        }}>
                            {action.title}
                        </h4>
                        <ul style={{ margin: 0, paddingLeft: 20 }}>
                            {action.actions.map((item, idx) => (
                                <li key={idx} style={{ fontSize: "13px", marginBottom: 4 }}>{item}</li>
                            ))}
                        </ul>
                    </div>
                ))}
            ...
            <Card title="RISK CONTRIBUTION BREAKDOWN" size="small" style={{ marginBottom: 16 }}>
                <Table
                    ...
                    columns={[
                        ...
                        { title: "Contribution", dataIndex: "contribution", key: "contribution", align: "right" as const },
                    ]}
                />
            </Card>
            ...
            <div style={{ fontSize: 11, color: "#999", textAlign: "center" }}>
                ...
                <p style={{ margin: "4px 0" }}>
                    This report is generated by Blockchain Anomaly AI for compliance and risk assessment purposes.
                </p>
                <p style={{ margin: "4px 0" }}>
                ...
                <p style={{ margin: "8px 0 0 0", fontSize: 10 }}>
                    Report ID: {transactionData.id}-{Date.now()} | Generated: {new Date().toLocaleString()}
                </p>
            </div>
        </div>
    );
